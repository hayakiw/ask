      ...
      <div class="col-md-8">
        <select class="form-control" name="staff">
          ...
          @foreach (App\Staff::all() as $staff)
            <option value="{{ $staff->id }}"@if (Request::old('staff', $item->staff_id) == $staff->id) selected="selected"@endif>{{ $staff->getName() }} : {{ $staff->email }}</option>
          @endforeach
        </select>
        @if ($errors->has('staff'))
        <p class="help-block"><span>{{ $errors->first('staff') }}</span></p>
        @endif
      </div>
    </div>
