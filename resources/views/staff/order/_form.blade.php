...
<div class="row">
  <div class="col-sm-6">
    <div class="form-group{{ $errors->has('ok') ? ' has-error' : '' }}">
      <label for="input_name">依頼受け入れ <span class="text-danger">※</span></label><br>
      <?php $okOld = Request::old('ok') ?: $order->ok;?>
      <label><input type="radio" name="ok" value="1"@if($okOld) checked="checked"@endif> 承諾する</label>
      <label><input type="radio" name="ok" value="0"> お断りする</label>
      @if ($errors->has('ok'))
...
<div class="row">
  <div class="col-sm-6">
    <div class="form-group{{ $errors->has('prefer') ? ' has-error' : '' }}">
      <label for="input_name">作業日時 <span class="text-danger">※</span></label><br>
...
<div class="row">
  <div class="col-sm-6">
    <div class="form-group{{ $errors->has('staff_comment') ? ' has-error' : '' }}">
      <label for="input_name">返信内容 <span class="text-danger">※</span></label>
      <p>必ずご記入ください</p>
